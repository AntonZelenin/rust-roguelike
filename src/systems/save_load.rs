use specs::prelude::*;
use specs::saveload::{SimpleMarker, SerializeComponents, DeserializeComponents, SimpleMarkerAllocator, MarkedBuilder};
use std::fs::File;
use std::path::Path;
use std::fs;
    ...
    let mut deleteme: Option<Entity> = None;
    {
        ...
        for (e, h) in (&entities, &helper).join() {
            ...
            *worldmap = h.map.clone();
            worldmap.tile_content = vec![Vec::new(); map::MAP_CELL_COUNT];
            deleteme = Some(e);
        }
